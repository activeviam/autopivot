...
import static com.av.autopivot.spring.RolesConfig.ROLE_TECH;
import static com.av.autopivot.spring.RolesConfig.ROLE_USER;
...
public class SecurityConfig implements ICorsConfig {
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		auth
				...
				.passwordEncoder(UserDetailsConfig.PASSWORD_ENCODER).and()
	...
	public abstract static class AWebSecurityConfigurer extends WebSecurityConfigurerAdapter {
		 ...
		 */
		protected AWebSecurityConfigurer() {
			this(null);
		}
		 ...
		 */
		protected AWebSecurityConfigurer(String cookieName) {
		 ...
		 * {@inheritDoc}
		 * <p>
		 * This configures a new firewall accepting `%` in URLs, as none of the core services encode information in
		 * URL. This prevents from double-decoding exploits.<br>
		 ...
		 * Yet, nor `/` and `.` - slash and point - are accepted, as it may trick the REGEXP matchers used for
		 * security. Support for those two characters can be added at your own risk, by extending this method. As
		 * far as ActiveViam APIs are concerned, `/` and `.` in URL parameters do not represent any risk. `;` -
		 * semi-colon - is also not supported, for various APIs end up target an actual database, and because this
		 * character is less likely to be used.
		 */
		@Override
		...
		protected final void configure(final HttpSecurity http) throws Exception {
			...
			http
					// The Cross Site Request Forgery protection is disabled to simplify the configuration
					// of the sandbox.
					// Applications without CSRF protection are at risk when performing actions based on
					// inputs from an authenticated user.
					// DO NOT copy this configuration to a production environment.
					.csrf()
					.disable()
					.cors()
					.and()
	...
	@Configuration
	@Order(6)
	public static class AdminUISecurityConfigurer extends AWebSecurityConfigurer {

		@Override
		protected void doConfigure(final HttpSecurity http) throws Exception {
			final String url = AdminUIResourceServerConfig.DEFAULT_NAMESPACE + "/**";
			http
					// Only theses URLs must be handled by this HttpSecurity
					.antMatcher(url)
					.authorizeRequests()
	 ...
	 *
	 * @author ActiveViam
	 * @see com.qfs.versions.service.IVersionsRestService
	 */
	@Configuration
	@Order(3)
	...
	public static class ActivePivotSecurityConfigurer extends AWebSecurityConfigurer {
		...
		protected void doConfigure(HttpSecurity http) throws Exception {
			http.authorizeRequests()
					...
					.hasAnyAuthority(RolesConfig.ROLE_USER, RolesConfig.ROLE_TECH)
					// REST services
					.antMatchers(REST_API_URL_PREFIX + "/**")
					.hasAnyAuthority(RolesConfig.ROLE_USER)
					// One has to be a user for all the other URLs
					.antMatchers("/**")
					.hasAuthority(RolesConfig.ROLE_USER)
					.and()
					.httpBasic()
