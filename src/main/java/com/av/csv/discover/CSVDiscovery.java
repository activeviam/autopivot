...
public class CSVDiscovery {
	...
	public CSVFormat discoverFile(String fileName, Charset charset) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(is, charset))) {
			...
			boolean quoteProcessing = detectQuoteProcessing(content, separator);
	...
	public boolean detectQuoteProcessing(List<String> rows, String separator) {
		boolean qp = false;
		for(String row : rows) {
			qp = qp || CSVSplitter.containsQuotedField(row, separator);
		}
		return qp;
	}
